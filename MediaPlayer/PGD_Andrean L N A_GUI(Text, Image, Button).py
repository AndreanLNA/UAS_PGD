...
class PemutarMusik(Frame):
    def __init__(self,parent):
        ...
        parent.geometry("640x280")
        self.parent = parent

        self.nama = StringVar()
        self.nama.set("Klik Open di bagian kiri untuk memasukkan lagu yang diinginkan")
    ...
    def tombolNext(self):
        next = Button(text="Lagu selanjutnya", command=self.next)
    ...
    def slider(self):
        w1 = Scale(from_=0.00, to=1.0,resolution=0.1, command=self.volume, orient=HORIZONTAL, length=500, label='Atur Volume Music :',showvalue=0)
        w1.pack()
        w1.set(0.50)
